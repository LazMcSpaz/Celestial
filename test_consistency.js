 ...
 *
 * "When the same underlying data is shown in more than one place, do all those
 *  places agree?" Boots the REAL app code (via test_harness) under a capturing
 * DOM shim, runs the actual render functions, and compares what each spot of the
 * UI ends up displaying across a (users × dates) sweep.
 *
...
const { boot, USERS } = require('./test_harness');
...
// each index maps to must never contradict.
{
...
for (const ukey of Object.keys(USERS)) {
  ...
  for (const date of DATES) {
    ...
    for (const id of spots) {
      ...
      if (/<em>\s*<\/em>/.test(raw)) dirty.push(`${id}:empty <em>`);
    }
